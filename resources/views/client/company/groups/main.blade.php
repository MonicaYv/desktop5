    ...
    <script>
        $(document).ready(function() {
            ...
            function populateTable(searchTerm = '', page = 1, clientId = '', companyId = '') {
                const listRoute = @json(route('client.company.group.list'));
                $.ajax({
                    url: listRoute,
                    method: 'GET',
                    data: {
            ...
            $('.addgroupmodel').on('click', function() {
                const addGroupRoute = @json(route('client.company.group.add')); 
            ...
            $(document).on('click', '#searchableTableBody .editGroupModal', function(e) {
                const editGroupRoute = @json(route('client.company.group.edit')); 
            ...
            $('#okdelete').on('click', function() {
                ...
                let deleteRoute = @json(route('client.company.group.delete')); // Route to delete the group
