    ...
    <script>
        $(document).ready(function() {
            ...
            function populateTable(searchTerm = '', page = 1, clientId = '', companyId = '') {
                const listRoute = @json(route('client.company.role.list'));
                $.ajax({
                    url: listRoute,
                    method: 'GET',
                    data: {
            ...
            $('.addrolemodel').on('click', function() {
                const addRoleRoute = @json(route('client.company.role.add')); 
                $.ajax({
                    url: addRoleRoute,
                    method: 'GET',
                    success: function(response) {
                        $('#addRoleModelDiv').html(response.html).show();
                    },
                    error: function(xhr) {
                        console.error(xhr.responseText);
                    }
            ...
            $(document).on('click', '#searchableTableBody .editRoleModal', function() {
                const editRoleRoute = @json(route('client.company.role.edit'));
                ...
                $.ajax({
                    ...
                    data: { roleid: roleId },
                    success: function(response) {
                        $('#editRoleModalDiv').html(response.html).show();
                    },
                    error: function(xhr) {
                        console.error(xhr.responseText);
                    }
            ...
            $('#okdelete').on('click', function() {
                ...
                const deleteRoute = @json(route('client.company.role.delete'));
                $.ajax({
                    url: deleteRoute,
                    method: 'POST',
                    data: {
