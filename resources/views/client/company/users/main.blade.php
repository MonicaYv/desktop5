    ...
    <div class="flex-grow h-100 main">
        <div class="flex w-full h-full flex-col content">
            ...
            <div class="px-4 py-3 md:px-6 md:py-4 flex gap-4 items-center bg-c-light-white-smoke">
                ...
                <select id="clientDropdown" class="w-full md:w-1/4 p-2 bg-white border border-c-gray rounded-md outline-none text-c-black">
                ...
                <label for="companyDropdown" class="font-semibold text-c-black hidden" id="companyLabel">Select Company:</label>
                <select id="companyDropdown" class="w-full md:w-1/4 p-2 bg-white border border-c-gray rounded-md outline-none text-c-black hidden">
                    ...
                    <table class="table-auto w-full">
                        <thead class="h-14">
                            <tr class="bg-c-dark-gray">
                                ...
                                <th class="text-c-white font-medium text-left pl-3 pr-3 md:pr-0">Client</th>
...
<script>
    $(document).ready(function() {
        ...
        populateTable();
        function populateTable(searchTerm = '', page = 1, clientId = '', companyId = '') {
            const listRoute = @json(route('client.company.user.list'));
            $.ajax({
                url: listRoute,
                method: 'GET',
                data: {
        ...
        clientDropdown.on('change', function() {
            ...
            if (clientId) {
                companyDropdown.removeClass('hidden');
                ...
                $.ajax({
                    ...
                    success: function(companies) {
                        companyDropdown.empty().append('<option value="">All Companies</option>');
                ...
                companyDropdown.addClass('hidden').empty();
        ...
        companyDropdown.on('change', function() {
            ...
            const searchTerm = $('#searchterm').val();
            populateTable(searchTerm, 1, clientId, companyId);
        });

        $('#searchterm').on('input', function() {
            const searchTerm = $(this).val();
            populateTable(searchTerm, 1, clientDropdown.val(), companyDropdown.val());
        });

        $(document).on('click', '.pagination a', function(e) {
            e.preventDefault();
            const page = $(this).attr('href').split('page=')[1];
            populateTable($('#searchterm').val(), page, clientDropdown.val(), companyDropdown.val());
        });

        // Add User Modal
        $('.addusermodel').on('click', function() {
            const addUserRoute = @json(route('client.company.user.add')); 
            $.ajax({
                url: addUserRoute,
                method: 'GET',
                success: function(response) {
                    $('#addUserModelDiv').html(response.html).show();
                },
                error: function(xhr) {
                    console.error(xhr.responseText);
                }
        ...
        $(document).on('click', '#searchableTableBody .editUserModal', function() {
            const editUserRoute = @json(route('client.company.user.edit'));
            ...
            $.ajax({
                ...
                data: { userid: userId },
                success: function(response) {
                    $('#editUserModalDiv').html(response.html).show();
                },
                error: function(xhr) {
                    console.error(xhr.responseText);
                }
        ...
        $('#okdelete').on('click', function() {
            ...
            const deleteRoute = @json(route('client.company.user.delete'));
            $.ajax({
                url: deleteRoute,
                method: 'POST',
                data: {
        ...
        $(document).on('change', '#allUserModel .clientchangelist', function() {
            ...
            $.ajax({
                ...
                success: function(companies) {
                    $('#allUserModel .companychangelist').empty().append('<option value="">All Companies</option>');
                    $.each(companies, function(index, company) {
                        const encodedId = btoa(company.id);
                        $('#allUserModel .companychangelist').append('<option value="' + encodedId + '">' + company.name + '</option>');
                    });
                },
                error: function(xhr) {
                    console.error(xhr.responseText);
         ...
         function fetchRolesAndGroups(clientId, companyId = null) {
            ...
            $(document).on('change', '#allUserModel .companychangelist', function (e) {
                const clientId = $('#allUserModel .clientchangelist').val();
                const companyId = $(this).val();
                fetchRolesAndGroups(clientId,companyId);
            });
