        ...
        <div class="flex w-full h-full flex-col content">
                ...
                <div class="relative taskicon hidden md:flex md:w-5/12 flex flex-row items-center justify-end gap-6">
                    <div id="searchbutton"
                        ...
                        <input type="text"
                            ...
                            placeholder="Search companies, Name & Username" id="searchterm" />
            ...
            <div class="pl-4 pt-3 mt-3 pb-3 pr-4 w-full flex flex-row justify-between items-center bg-c-light-white-smoke md:hidden"
                ...
                <div class="relative w-full flex flex-row items-center justify-end gap-2">
                    ...
                    <button class="px-2 has-tooltip addcompanymodel" >
                        <i class="ri-add-circle-fill ri-xl"></i>
                    </button>
                    <div
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#allCompanyModel .togglePassword', function(e) {
            ...
            $('.addcompanymodel').on('click', function() {
                $.ajax({
                    url: @json(route('company.add')),
                    method: 'GET',
                    success: function(response) {
                        $('#addCompanyModelDiv').html(response.html).show();
                    },
                    error: function(xhr) {
                        console.error(xhr.responseText);
                    }
            ...
            $(document).on('click', '#searchableTableBody .editCompanyModal', function() {
                ...
                $.ajax({
                    url: @json(route('company.edit')),
                    method: 'GET',
                    data: { companyid: companyId },
                    success: function(response) {
                        $('#editCompanyModalDiv').html(response.html).show();
                    },
                    error: function(xhr) {
                        console.error(xhr.responseText);
                    }
