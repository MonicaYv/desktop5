    ...
    <script>
        $(document).ready(function() {
            ...
            function populateTable(searchTerm = '', page = 1) {
                const listroute = @json(route('groups.list'));
                $.ajax({
                    url: listroute,
                    method: 'GET',
                    data: {
            ...
            $('.addgroupmodel').on('click', function() {
                const addGroupRoute = @json(route('groups.add')); 
            ...
            $(document).on('click', '#searchableTableBody .editGroupModal', function(e) {
                const editGroupRoute = @json(route('groups.edit')); 
            ...
            $('#okdelete').on('click', function() {
                ...
                let deleteRoute = @json(route('groups.delete')); // Route to delete the group
